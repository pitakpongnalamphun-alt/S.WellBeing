...
/**
 * การ์ดชวนคุยกับน้องปุยบนหน้าแรก
 *
 * เดิมเป็นแถวลิงก์ธรรมดาที่ใช้ไอคอน MessageCircleHeart ของ lucide ทั้งที่ทั้งฟีเจอร์
 * คือ *ตัวละคร* — เด็กที่เลี้ยงปุยเป็นมู้ดเพ็ต ฝึกหายใจกับปุย และแต่งตัวให้ปุย
 * มาเจอการ์ดที่ไม่มีหน้าปุยอยู่ในนั้นเลย ตอนนี้ใช้ Puy ตัวเดียวกับในห้องแชท
 * พร้อมของตกแต่งที่เด็กแต่งไว้เอง
 *
 ...
 * เรื่องที่เด็กเล่าให้ปุยฟังคนเดียว กลายเป็นของที่คนข้าง ๆ อ่านได้โดยที่เขาไม่ได้ตั้งใจ
 */
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          <span className="block text-[1.02rem] font-bold leading-snug text-ink">
            {continuing ? "กลับมาคุยกับปุยต่อไหม" : "วันนี้อยากเล่าอะไรให้ปุยฟังไหม"}
          </span>
          <span className="mt-0.5 block text-[0.78rem] leading-snug text-ink-soft">
            {continuing
              ? ago
                ? `คุยค้างไว้${ago === "เมื่อสักครู่" ? "" : "เมื่อ"}${ago} · ปุยยังอยู่ตรงนี้`
                : "ปุยยังจำบทสนทนาที่ค้างไว้ในเครื่องนี้ได้"
